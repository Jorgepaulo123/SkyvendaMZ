...
export default function NhonguistasCarousel({ embedded=false }) {
  ...
  return (
    <div className={`${embedded ? '' : 'container mx-auto'} my-8`}>
      <div className="bg-gradient-to-r from-cyan-50 to-blue-50 rounded-lg px-6 py-5 relative overflow-hidden">
        ...
        <div ref={containerRef} className="flex gap-4 overflow-x-auto pb-2 scroll-smooth snap-x scrollbar-hide">
          {vendedores.map(v => (
            <div
              key={v.id}
              className="min-w-[120px] snap-start flex-shrink-0 cursor-pointer text-center"
              onClick={() => navigate(`/${v.username || v.identificador_unico || v.id}`)}
            >
              <img
                src={v.foto_perfil || v.avatar || v.foto || 'https://via.placeholder.com/100'}
                alt={v.nome || v.name}
                className="w-[100px] h-[100px] rounded-full object-cover border-2 border-indigo-500 mb-2"
